import { AppConfigPath, FeatureToggle, HttpMethod } from './types';
import { enc, HmacSHA256, SHA256 } from 'crypto-js';
import axios, { AxiosRequestConfig } from 'axios';
import { getFunctionAppConfig } from './utils';

function signRequest(host: string, method: HttpMethod, url: string, credential: string, secret: string, body: string = '') {
  ...
  const headersSignature = HmacSHA256(headerValues, enc.Base64.parse(secret)).toString(enc.Base64);
...
async function sendGetRequestToAppConfig(host: string, path: AppConfigPath, responseTransformer: (data: string) => FeatureToggle[]) {
  ...
  const headers = signRequest(host, HttpMethod.GET, path, accessKeyId, accessKeySecret);
