const { STG_UI_USERNAME } = process.env
const { STG_UI_PASSWORD } = process.env

export const loginCredentials = {
    username: STG_UI_USERNAME || new Error('username not found'),
    password: STG_UI_PASSWORD || new Error('password not found')
}
