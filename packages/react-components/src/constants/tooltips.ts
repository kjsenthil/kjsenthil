...
export const DataPeriodTooltip = (dataPeriod: string): string => {
  dataPeriod =
    dataPeriod === "year's" || dataPeriod === "month's"
      ? dataPeriod.slice(0, -2)
      : dataPeriod.slice(0, -1);
  return `This shows how well your investments have performed in the last ${dataPeriod.toLowerCase()}. It includes returns from both growth and income, minus any fees.`;
};
