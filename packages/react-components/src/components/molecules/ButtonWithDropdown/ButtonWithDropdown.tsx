...
export interface ButtonWithDropdownProps extends Omit<ButtonProps, 'small'> {
  label: string;
...
const ButtonLineSeparator = styled.div`
  width: 2px;
  height: 40px;
  z-index: 1;
  ${({ color, variant, theme }: Pick<ButtonProps, 'color' | 'variant'> & { theme: Theme }) => {
    ...
    >
      <ButtonWithDropdownWrapper>
        <div>
          ...
          <ButtonLineSeparator color={props.color} variant={props.variant} />
