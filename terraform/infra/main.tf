...
module "api_management_policy" {
  ...
  xml_policy_file = templatefile("../modules/policy_documents/api_operation_policy.xml", {
    config = {
      ...
      cors_allowed_origins = coalescelist(lookup(each.value.policy.cors, "allowed_origins", []), [var.environment_prefix == "prod" ? "https://my.bestinvest.co.uk" : "", var.environment_prefix == "staging" ? "https://my.demo2.bestinvest.co.uk" : "", var.environment_prefix != "prod" ? "http://localhost:8000" : "", var.environment_prefix != "prod" ? module.front_end.web_endpoint : "", coalesce(format("https://%s", module.front_end.website_cname_record[0]), "")]),
...
module "api_management_policy_xplan" {
  ...
  xml_policy_file = templatefile("../modules/policy_documents/api_operation_policy_xplan.xml", {
    config = {
      ...
      cors_allowed_headers = lookup(each.value.policy.cors, "headers", null),
      cors_exposed_headers = lookup(each.value.policy.cors, "expose_headers", null),
      cors_allowed_origins = coalescelist(lookup(each.value.policy.cors, "allowed_origins", []), [var.environment_prefix != "prod" ? "http://localhost:8000" : "", var.environment_prefix != "prod" ? module.front_end.web_endpoint : "", coalesce(format("https://%s", module.front_end.website_cname_record[0]), "")]),
      allow-credentials    = lookup(each.value.policy.cors, "allow-credentials", false),
      headers              = lookup(each.value.policy, "set_header", null),
      outbound_headers     = lookup(each.value.policy, "outbound_headers", null),
      rewrite_url          = lookup(each.value.policy, "rewrite_url", null),
