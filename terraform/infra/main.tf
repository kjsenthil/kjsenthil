...
module "api_management_policy" {
  ...
  xml_policy_file = templatefile("../modules/policy_documents/api_operation_policy.xml", {
    config = {
      ...
      cors_allowed_origins = coalescelist(lookup(each.value.policy.cors, "allowed_origins", []), ["https://my.bestinvest.co.uk", var.environment_prefix != "prod" ? "http://localhost:8000" : "", var.environment_prefix != "prod" ? module.front_end.web_endpoint : "", coalesce(format("https://%s", module.front_end.website_cname_record[0]), "")]),
